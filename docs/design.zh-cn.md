...
| 限制 | 描述 | 临时方案 |
|------|------|---------|
| 数字列头 | 列头为数字（如 `Week 1, 2, 3...52`）时，字符串占比检测（≥70%）不通过，表头无法识别 | 调低 `header_min_string_ratio` 或重新整理 Excel |
| 稀疏表头行 | 表头填充率 ≤50%（如 4 列只填了 2 列）被填充率检测拒绝，边界判断为严格 `>`  | 调低 `header_min_fill_ratio` |
| 空分隔列 | Excel 中数据列之间的空白列会增大网格面积，拉低 island 检测置信度，且空列会作为全 NULL 列写入 DB | 后处理删除全 NULL 列 |
